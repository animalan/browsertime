...
export class Firefox {
  ...
  async beforeBrowserStart() {
    ...
    if (isAndroidConfigured(this.options) && this.options.androidSimpleperf) {
  ...
  async beforeEachURL(runner, url) {
    ...
    if (isAndroidConfigured(this.options)) {
      ...
      if (
        this.options.androidSimpleperf &&
        this.options.androidSimpleperfRecordingType !== 'custom' &&
        this.options.androidSimpleperfRecordingType !== 'browser'
      ) {
        this.simpleperf = new Simpleperf(this.storageManager, this.options);

        await this.simpleperf.start();
      }
    }
